...
(defn silent-login
  ...
  (ajax/ajax-request
    {:method          :post
     :uri             (str base-url "/backsite-api/v1/user/silent-login")
...
(defn login
  ...
  (ajax/ajax-request
    {:method          :post
     :uri             (str base-url "/backsite-api/v1/user/login")
     :format          (ajax/json-request-format)
     :response-format (ajax/json-response-format {:keywords? true})
     :params          cred
     :handler         (fn [[status data]]
...
(defn signup
  ...
  (ajax/ajax-request
    {:method          :post
     :uri             (str base-url "/backsite-api/v1/user/signup")
     :format          (ajax/json-request-format)
     :response-format (ajax/json-response-format {:keywords? true})
     :params          user-data
     :handler         (fn [[status data]]
...
(defn api-health-check []
  (ajax/ajax-request
    {:method          :get
     :uri             (str base-url "/backsite-api/v1/user")
     :format          (ajax/json-request-format)
     :response-format (ajax/json-response-format {:keywords? true})
     :handler         (fn [[_ data]]
                        (js/console.log data))
...
(defn init-authorise []
  ...
  (if-let [local-cred (reader/read-string (u/get-storage "cred"))]
    (re/dispatch [:start-event/silent-login local-cred])
    (re/dispatch [:navigate :login])))
